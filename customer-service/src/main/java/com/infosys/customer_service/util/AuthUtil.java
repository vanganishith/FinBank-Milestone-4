package com.infosys.customer_service.util;

import com.infosys.customer_service.exception.AccessDeniedException;
import com.infosys.customer_service.feign.AuthFeignClient;
...
public class AuthUtil {
    ...
    public Integer requireCustomerAndGetId(String authHeader) {
        Map<String, Object> claims = validateAndGetClaims(authHeader);
        String role = (String) claims.get("role");
        if (!"CUSTOMER".equals(role)) {
            throw new AccessDeniedException("Access denied. Customers only.");
        }
        Object custIdClaim = claims.get("custId");
        if (custIdClaim == null) {
            throw new AccessDeniedException("Token missing customer id.");
        }
        return ((Number) custIdClaim).intValue();
    }
}